...
public class Waterfles {
	...
	public void vulBij(int hoeveelheidWater) {
		if (hoeveelheidWater <= 0)
			throw new IllegalArgumentException("Hoeveelheid water moet strikt positief zijn!");

		if (this.inhoud + hoeveelheidWater > this.capaciteit) {
			hoeveelheidWater = this.capaciteit;
		} else {
			hoeveelheidWater += this.inhoud;
		}

		setInhoud(hoeveelheidWater);
	}

	private void controleerCapaciteit(int capaciteit) {
		if (capaciteit < MIN_CAPACITEIT || capaciteit > MAX_CAPACITEIT)
			throw new IllegalArgumentException(
					String.format("Capaciteit moet in het interval [%%d, %d] liggen!", MIN_CAPACITEIT, MAX_CAPACITEIT));
	}
